...
function executeTest(acceptanceTests, index) {
    if (! acceptanceTests[index]) {
        console.log('Finished');
        return;
    }
    var test = acceptanceTests[index];
    if (test.scenario) {
        test.execute(function(err, result) {
            if (err) {
                console.log('Error:');
                console.log(err);
            } else {
                ...
                // console.log(result);
                // console.log('----');
                executeTest(acceptanceTests, index + 1);
            }
        });
    } else {
        executeTest(acceptanceTests, index + 1);
...
exports.launchAll = function(req, res, next) {
...
function list(query, req, res, next) {
    ...
    if (req.query.status) {
        var status = _.isArray(req.query.status) ? req.query.status : [req.query.status];
        query.where('currentStatus').in(status);
    }
...
exports.listPublic = function (req, res, next) {
    ...
    query.populate('user', 'organization -_id')
        .where('state', 'validated')
        .where(_.pick(req.query, 'user'))
        .select('-rejectionMessage -state -priority');
