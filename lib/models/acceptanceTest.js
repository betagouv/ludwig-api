    ...
    execute: function(done) {
        ...
        function buildDroit(code, result) {
            ...
            var droitSimule = {
                code: code,
                result: result,
                expectedValue: droitAttendu.expectedValue,
                status: droitAttendu.expectedValue === result ?
                        'accepted-exact' :
                        'rejected'
            };

            if (typeof droitSimule.expectedValue === 'number' &&
                typeof result === 'number' &&
                droitSimule.expectedValue !== 0) {
                droitSimule.percentDiff = Math.abs((droitAttendu.expectedValue - result) / droitAttendu.expectedValue);

                if (droitSimule.percentDiff <= 0.0001)
                    droitSimule.status = 'accepted-exact';
                else if (droitSimule.percentDiff <= 0.02)
                    droitSimule.status = 'accepted-2pct';
                else if (droitSimule.percentDiff <= 0.1)
